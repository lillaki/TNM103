...
void fillSramBufferWithWaveTable(){
  ...
  /* Sinusvåg
  float SoundValue = 0;
  float deltaSoundValue = 2*M_PI/512;
  for(int i = 0; i< 511; i++){
     float sinusSample = 127 * sin(SoundValue);
  ...
  float SoundValue;
  for(int i = 0; i<512; i++){
    SoundValue = 64 + floor(i/2.0);
    if(i > 255){
      SoundValue = 192 + floor(i/2.0);
    }
    sramBuffer[i] = SoundValue/2;
  }
}
